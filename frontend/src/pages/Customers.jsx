        ...
        {list.map(c=>(
          <div key={c.phone} className="border rounded-2xl p-4 bg-white">
            ...
            <div className="flex justify-between mt-2 bg-orange-50 p-2 rounded-xl">
              <span>VISITS {c.visits}</span><span className="font-bold text-orange-600">₹{c.totalSpent}</span>
            </div>
            <div className="text-xs mt-1">Last: {new Date(c.lastVisit).toLocaleDateString('en-IN')}</div>
          </div>
        ))}
      </div>
    </div>
